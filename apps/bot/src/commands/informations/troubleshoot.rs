...
/// Help users troubleshoot errors for their servers.
...
impl Command<App> for TroubleshootCommand {
    ...
    async fn handle_command(&self, ctx: &CommandContext<'_>) -> CommandResult {
        match build_troubleshoot_menu(&ctx.i18n_locale(), "home") {
            ...
            Err(err) => {
                ctx.say(format!("Error loading troubleshoot menu: {err}"))
                    .await?;
            }
        }
        Ok(())
        ...
        if let Some(InteractionData::MessageComponent(data)) = ctx.interaction.data.as_ref() {
            ...
            match build_troubleshoot_menu(&ctx.i18n_locale(), path) {
                ...
                Err(err) => {
                    ctx.reply(
                        Reply::new()
                            .content(format!("Error loading troubleshoot menu: {err}"))
                            .ephemeral(true),
                    )
                    .await?;
                }
...
fn build_troubleshoot_menu(locale: &Locale, path: &str) -> Result<Reply> {
    let file_content = match locale {
        Locale::Fr => include_str!("../../../locales/fr/troubleshoot.json"),
        Locale::De => include_str!("../../../locales/de/troubleshoot.json"),
        Locale::Es => include_str!("../../../locales/es/troubleshoot.json"),
        Locale::Lt => include_str!("../../../locales/lt/troubleshoot.json"),
        _ => include_str!("../../../locales/en/troubleshoot.json"),
    };

    let mut current_node: Value =
        serde_json::from_str(file_content).context("Failed to parse troubleshoot.json")?;
        ...
        row.add_select_menu(|menu| {
            ...
            if path != "home" {
                ...
                menu = menu.add_option(|opt| {
                    opt.label(t!(locale, "commands.troubleshoot.return"))
                        .value(parrent_path)
                });
            }
            menu
